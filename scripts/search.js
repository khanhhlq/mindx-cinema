...
(function () {
  if (query) {
    fetch('https://api.themoviedb.org/3/search/movie?query=' + encodeURIComponent(query) + '&api_key=' + TMDB_API_KEY).then(function (response) {
      ...
      movieGridHTML += '<div>';
      movieGridHTML += data.results.map(function (item) {
        return '<a href="./info.html?id=' + item.id + '">' + '<div class="movie-card">' + '<img style="width: auto; height: auto; aspect-ratio: 2/3" class="fade-in" onload="this.style.opacity = \'1\'" src="https://image.tmdb.org/t/p/w200' + item.poster_path + '" alt=""/>' + '<p class="multiline-ellipsis-2">' + (item.title || item.name) + '</p>' + '</div>' + '</a>';
      }).join('');
      movieGridHTML += '</div>';
