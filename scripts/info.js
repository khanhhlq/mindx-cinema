...
(function () {
  ...
  Promise.all([fetch('https://api.themoviedb.org/3/movie/' + movieId + '?api_key=' + TMDB_API_KEY).then(function (response) {
    return response.json();
  }), fetch('https://api.themoviedb.org/3/movie/' + movieId + '/credits?api_key=' + TMDB_API_KEY).then(function (response) {
    return response.json();
  }), fetch('https://api.themoviedb.org/3/movie/' + movieId + '/similar?api_key=' + TMDB_API_KEY).then(function (response) {
    return response.json();
  })]).then(function (results) {
    // Reduce the results to a final object
    var result = results.reduce(function (_final, current, index) {
      if (labels[index] === "data") {
        _final[labels[index]] = current;
      } else if (labels[index] === "casts") {
        _final[labels[index]] = current.cast.filter(function (item) {
          return item.name && item.character && item.profile_path;
        }).slice(0, 10);
      } else if (labels[index] === "similar") {
        _final[labels[index]] = current.results;
      }
      return _final;
    }, {});
    console.log(result);
    ...
    if (result.data.genres) {
      document.querySelector("#genres").innerHTML = result.data.genres.map(function (genres) {
    ...
    if (result.casts) {
      document.querySelector(".casts-grid").innerHTML = result.casts.map(function (item) {
        return '<div>' + '<img onload="this.style.opacity = \'1\'" class="fade-in" src="https://image.tmdb.org/t/p/w200' + item.profile_path + '" alt="">' + '<p style="text-align: center">' + item.name + '</p>' + '<p style="text-align: center; color: var(--orange)">' + item.character + '</p>' + '</div>';
      }).join('');
    }

    // Set similar movies section if available
    if (result.similar && result.similar.length > 0) {
      document.querySelector("#similar").innerHTML += '<div class="section">' + '<h2>Similar</h2>' + '<div class="swiper">' + '<div class="swiper-wrapper">' + result.similar.map(function (item) {
        return '<a href="./info.html?id=' + item.id + '" class="swiper-slide" style="width: 200px !important">' + '<div class="movie-card">' + '<img class="fade-in" onload="this.style.opacity = \'1\'" src="https://image.tmdb.org/t/p/w200' + item.poster_path + '" alt="">' + '<p class="multiline-ellipsis-2">' + (item.title || item.name) + '</p>' + '</div>' + '</a>';
